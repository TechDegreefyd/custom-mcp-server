...
**Live counts (validated):** DIP=124, Enrolled=123, Onboarded=11, Activated=295, Refunded=18 | Total=571
**Sessions:** Jan 2026 (328 students), July 2026 (243 students)
...
### students
One row per enrolled student. Central record. 571 students.
...
- `assigned_servicing_counsellor_id` → servicing team (**430/571 students assigned**)
- `assigned_documentation_counsellor_id` → documentation team (**571/571 — all students assigned**)
- `assigned_placement_counsellor_id` → placement team (**only 14/571 assigned — very sparse**)
...
- ⚠️ `IDs Created` is **NOT** a `dip_sub_stage` value on the `students` table — it exists only as a `stage` in `student_workflows` (documentation team, 350 rows). It represents university/LMS credentials being created after a student leaves DIP. Never use it as a `dip_sub_stage` filter on `students`.
...
### users
One row per CRM staff member. 16 users.
...
- Filter active staff: `is_active = true` — **1 user is inactive** (15 active, 1 inactive)
...
### student_workflows
Append-only audit log of every workflow stage transition. 5,202 rows. Multiple rows per student.
...
- **documentation**: Docs Pending, Docs Submitted, Under Verification, Enrolled_L1 Checked, E-sign Pending, Resubmitted, Results Awaited, IDs Created, Rework Required, Refund Applied
  - `IDs Created` exists here (350 rows) but NOT in `students.dip_sub_stage` — student has already left DIP by this point
...
- `status = 'active'` = stage currently in progress (672 rows); `status = 'completed'` = stage done (4,531 rows)
...
- **All 571 students have at least one workflow row** — no gaps
...
### remarks
Call/interaction log. 2,766 rows. Multiple rows per student.
...
- `is_task = true` rows are tasks, not calls — **247 of 2,766 rows are tasks (9%)**. Always exclude from call metrics unless user asks
...
- **570/571 students have at least one call remark; 1 student has never been called**
...
- `call_type`: phone=2764, video=7 — video calls are rare

---

### student_status_histories
Append-only audit of lead_status changes. 386 rows.
...
- **⚠️ 304/571 students have NO status history rows** — this table is incomplete. Do NOT use it to count all students at a status; use `students.lead_status` for that

---

### enrollment_approvals
Manager approval/rejection records for enrollments. 431 rows.
...
- **Validated: 430 approved, 1 rejected** (99.8% approval rate currently)
- **Only 1 counsellor (Mohd Uvais) has submitted approvals** — `performed_by` is sparse across team

---

### documents
Student documents uploaded for enrollment. 261 rows.
...
- **Validated counts:** Academic=112, KYC=71, Photo=37, L1 Checked Document=36, Work=5
- **Only 261 docs for 571 students** — many students have no documents uploaded yet

---

### counsellor_tasks
Pending/completed tasks per counsellor. 1,175 rows.
...
- **Validated task breakdown:** Onboarding (completed=307, pending=123), Activation (completed=296, pending=9), General Checkin (pending=430 — none completed), Placement Follow up (pending=10)
- **General Checkin has 0 completed tasks** — all 430 are pending. Do not use completion rate for General Checkin
- `assigned_team` can be `servicing` or `placement` — Placement Follow up appears under both

---

### student_finances
One row per student's financial record. 571 rows (one per student).
...
- `total_payment_credited` = total money received from university — **currently 0 for ALL students** (no payments received yet)
- `deficit_amount` = amount still pending from university — **total outstanding = ₹2.85 Cr across all students**
- `is_eligible_for_payment`: 439 students eligible, 132 not yet eligible
- `is_eligible_for_payment = true` means university has cleared the student for payment
...
EMI/installment schedule per student. 1,174 rows (~2 per student).
...
Locked fee/commission structure at time of enrollment. 572 rows.
...
Breakdown of revenue components per finance record. 1,142 rows.
...
### course_fee_master
Master catalog of course fees per university. 291 rows.
...
### college_onboardings
University partnership config. 9 rows (one per university).
...
### tickets
Student support tickets. 44 rows.
...
### counsellor_tasks
*(See above under TABLES)*

---

### notifications
System notifications per user. 44,883 rows. High volume — avoid full scans.
...
**S10.** `student_status_histories` is **incomplete** — only 267/571 students have rows. NEVER count students from this table. Use it only for date-of-change lookups on students where it exists.
...
-- ⚠️ student_status_histories is INCOMPLETE (only 267/571 students have rows).
-- Use this pattern only as a best-effort; it will undercount.
...
-- **Validated against app export: 34 students, June 4 2026. 100% match.**
...
--    → Validated: Sikkim student had Docs Pending + Docs Submitted + L1 Checked created same day = counted as 3
...
### PATTERN 15 — Connectivity Metrics Report (Validated 2026-06-04)
...
-- Validated 100% match June 2026. Total: 575 students / 149.57L
```sql
SELECT
    s.university,
...
-- current_status values: 'To be Invoiced' | 'Ready to invoice' | 'Invoiced- Payment Pending' | 'Invoiced - Payment done' | 'Refunded Amount'
-- Validated 100% match June 2026. Total: 575 / 149.57L | Done: 285 / 67.58L | Pending: 44 / 7.91L
```sql
SELECT
    s.university,
    COUNT(DISTINCT s.id) AS total_count,
...
### PATTERN 18 — Overall Snapshot Dashboard (Validated June 4 2026)
-- Overall   = sf.current_status != 'Refunded Amount' → 538 students / ₹2.35Cr / ₹1.37Cr
...
-- YTD       = EXTRACT(YEAR FROM s.admission_taken_date) = 2026 AND excl Refunded Amount (~1 gap vs app)
